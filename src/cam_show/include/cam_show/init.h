...
bool r_errinit = true;
Point origin;
Rect selection, selection2,selection3,selection4,selection5,selection6;                                             //ROI矩形区域
Rect standard_window;
float dert = 33;
float  prop;
float  width_stan=0 , high_stan=0;
float one_two=0 , two_three=0 , three_four=0 , four_five=0 , five_six=0;
int vmin = 10, vmax = 256, smin = 90;   ///   v:亮度
int data=4;
float r1,r2,r3;     ///6个点坐标
float r1_err=0,r2_err=0,r3_err=0;
float diff_missing = 0;  ///目标丢失后，认为增加所搜索框放大尺度
float diff_premiss = 0;   ///目标即将丢失时，认为增加所搜索框放大尺度
float  ch_x,ch_y;
float expend_x,expend_y;
float len12_x , len12_y,len23_x , len23_y,len34_x , len34_y,len45_x , len45_y,len56_x , len56_y;
float v1_x=0 , v1_y=0 ,v2_x=0 , v2_y=0 ,v3_x=0 , v3_y=0 ,v4_x=0 , v4_y=0 ,v5_x=0 , v5_y=0, v6_x=0 , v6_y=0;
Rect trackWindow ,trackWindow2,trackWindow3,trackWindow4,trackWindow5,trackWindow6;//搜索框矩形
...
float trackcenter2_x,trackcenter2_y;
float trackcenter3_x,trackcenter3_y;
float trackcenter4_x,trackcenter4_y;
float trackcenter5_x,trackcenter5_y;
float trackcenter6_x,trackcenter6_y;


Point a,b,c,d,e,f;                                   //区域质心
//geometry_msgs::Point aa,bb,cc,dd;                //质心坐标
FILE *fp;
Mat frame, hsv, hue, mask,hist, hist2, hist3, hist4, hist5, hist6;
Mat histimg = Mat::zeros(200, 320, CV_8UC3), backproj,backprojj;
Mat histimg2 = Mat::zeros(200, 320, CV_8UC3), backproj2,backprojj2;
Mat histimg3 = Mat::zeros(200, 320, CV_8UC3), backproj3,backprojj3;
Mat histimg4 = Mat::zeros(200, 320, CV_8UC3), backproj4,backprojj4;
Mat histimg5 = Mat::zeros(200, 320, CV_8UC3), backproj5,backprojj5;
Mat histimg6 = Mat::zeros(200, 320, CV_8UC3), backproj6,backprojj6;

Mat m_predict;

deque<Rect> dequeWindow1;
deque<Rect> dequeWindow2;
deque<Rect> dequeWindow3;
deque<Rect> dequeWindow4;
deque<Rect> dequeWindow5;
deque<Rect> dequeWindow6;
...
static void onMouse(int event, int x, int y, int, void*)
{
    if (selectObject)//鼠标左键按下时，则条件为true，在if里面的代码块就是确定所选择的矩形区域selection
    {
        ...
        if (count_track == 2)
        {
            ...
            selection2.y = MIN(y, origin.y);//矩形左上角顶点的坐标
            selection2.width = std::abs(x - origin.x);//矩形宽
            selection2.height = std::abs(y - origin.y);//矩形高
            selection2 &= Rect(0, 0, image.cols, image.rows);//用于确保所选的矩形区域在图片区域内

        }
        if (count_track == 3)
        {
            selection3.x = MIN(x, origin.x);
            selection3.y = MIN(y, origin.y);//矩形左上角顶点的坐标
            selection3.width = std::abs(x - origin.x);//矩形宽
            selection3.height = std::abs(y - origin.y);//矩形高
            selection3 &= Rect(0, 0, image.cols, image.rows);//用于确保所选的矩形区域在图片区域内

        }
        if (count_track == 4)
        {
            selection4.x = MIN(x, origin.x);
            selection4.y = MIN(y, origin.y);//矩形左上角顶点的坐标
            selection4.width = std::abs(x - origin.x);//矩形宽
            selection4.height = std::abs(y - origin.y);//矩形高
            selection4 &= Rect(0, 0, image.cols, image.rows);//用于确保所选的矩形区域在图片区域内

        }
        if (count_track == 5)
        {
            selection5.x = MIN(x, origin.x);
            selection5.y = MIN(y, origin.y);//矩形左上角顶点的坐标
            selection5.width = std::abs(x - origin.x);//矩形宽
            selection5.height = std::abs(y - origin.y);//矩形高
            selection5 &= Rect(0, 0, image.cols, image.rows);//用于确保所选的矩形区域在图片区域内

        }
        if (count_track == 6)
        {
            selection6.x = MIN(x, origin.x);
            selection6.y = MIN(y, origin.y);//矩形左上角顶点的坐标
            selection6.width = std::abs(x - origin.x);//矩形宽
            selection6.height = std::abs(y - origin.y);//矩形高
            selection6 &= Rect(0, 0, image.cols, image.rows);//用于确保所选的矩形区域在图片区域内
    ...
    switch (event)//鼠标事件
    {
        case EVENT_LBUTTONDOWN:
            ...
            if(count_track == 2)
                selection3 = Rect(x, y, 0, 0);//鼠标刚按下去时初始化了一个矩形区域
            if(count_track == 3)
                selection4 = Rect(x, y, 0, 0);//鼠标刚按下去时初始化了一个矩形区域
            if(count_track == 4)
                selection5 = Rect(x, y, 0, 0);//鼠标刚按下去时初始化了一个矩形区域
            if(count_track == 5)
                selection6 = Rect(x, y, 0, 0);//鼠标刚按下去时初始化了一个矩形区域
            selectObject = true;
            count_track ++;
            break;
        case EVENT_LBUTTONUP:
            selectObject = false;
            if (selection.width > 0 && selection.height > 0 && count_track == 6)
                trackObject = -1;
            break;
    }
}
